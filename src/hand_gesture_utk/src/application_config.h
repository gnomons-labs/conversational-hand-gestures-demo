...
#define CAMERA_IMAGE_ALLOCATION             ALLOCATE_TO_SDRAM  /* Option: OnchipRAM or SDRAM */
...
/* BOTH CONSOLE DUMP SWITCHES ARE 0. The demo shares one console with the story text, so a
 * per-frame dump of every detection box and every stage timing would bury it. Nothing reads
 * them today - the only file that did, src\cam_thread_entry.c, has been deleted - but the
 * value is set here so that reviving that code cannot bring the noise back. */
#define ENABLE_AI_INFERENCE_RESULT_CONSOLE_OUTPUT    (0) // 0: Disabled, 1: Enabled
#define ENABLE_PROCESSING_TIME_RESULT_CONSOLE_OUTPUT (0) // 0: Disabled, 1: Enabled
