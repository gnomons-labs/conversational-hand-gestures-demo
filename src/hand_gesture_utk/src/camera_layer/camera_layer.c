...
#include "app_os.h"        /* app_os_delay_ms */
...
/* Written inside cam_vin_callback, which is an interrupt, and read in
 * camera_capture_post_process, which runs on T_CAM. The rule is that anything
 * shared with an interrupt carries volatile, so the compiler cannot cache it in a register
 * across the read. It is one aligned word, so the qualifier is all that is needed. */
static uint8_t * volatile p_camera_capture_buffer_stored;
...
void cam_vin_callback(capture_callback_args_t *p_args)
{
    ...
    if(event == VIN_EVENT_NOTIFY)
    {
        if (interrupt_status.bits.frame_complete)
        {
            ...
            /* The bit name changes and the operating-system call moves
             * behind app_sig_set_isr. The buffer-address store above and the frame-period
             * measurement stay. The yield check and the dropped-bit count now live in one place,
             * src\app_signal.c. */
            app_sig_set_isr(EV_CAM_FRAME);
 ...
 * generic short-packet first-in-first-out queue. Only the data-lane and virtual-channel ones are
 * true faults; the other three are counted as well, because a source that suddenly stops is as
 * useful a clue as one that starts.
 *********************************************************************************************************************/
static volatile uint32_t g_mipi_csi_events[CAM_MIPI_CSI_EVENT_SOURCES] = {0U};
...
void cam_mipi_csi_callback(mipi_csi_callback_args_t *p_args)
{
    ...
    if ((uint32_t) p_args->event < (uint32_t) CAM_MIPI_CSI_EVENT_SOURCES)
    {
        g_mipi_csi_events[(uint32_t) p_args->event]++;
    }
}

uint32_t camera_mipi_csi_event_count(uint32_t source)
{
    if (source >= (uint32_t) CAM_MIPI_CSI_EVENT_SOURCES)
    {
        return 0U;
    }

    return g_mipi_csi_events[source];
}

uint32_t camera_mipi_csi_error_count(void)
{
    return g_mipi_csi_events[MIPI_CSI_EVENT_DATA_LANE] +
           g_mipi_csi_events[MIPI_CSI_EVENT_VIRTUAL_CHANNEL];
}
