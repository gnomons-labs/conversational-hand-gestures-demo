...
/* NOT in the original mtk3_bsp2 source. Added for this port (sys_start.c).
 * Pushes the writes to knl_exctbl out of the data cache into memory, so the
 * processor's vector fetch can see them. See the comment on the definition. */
IMPORT void knl_flush_exctbl(void);
