...
/* 1 ms, to match the source project's FreeRTOS configTICK_RATE_HZ = 1000.
 * History: this was briefly put back to the shipped default of 10 while chasing a boot
 * fault. That fault turned out to be unrelated - the data cache versus mtk3's relocated
 * vector table, fixed in sysdepend/ra_fsp/cpu/core/armv8m/sys_start.c - so the value is
 * back to the one the port plan specifies.
 * Why 10 was not harmless: console_output_write() waits for TX_COMPLETE with
 * "while(!g_transfer_complete) tk_dly_tsk(1);", and generate() prints every token. With a
 * 10 ms tick each of the 256 token prints sleeps a full 10 ms granule instead of 1 ms,
 * which cost about 1.6 s over a 65 s run (3.86 tokens/s instead of the 3.95 baseline). */
 ...
 *  1: Valid  0: Invalid
 */
/* was: (1). Turned off for this port. T-Monitor's serial driver
 * (sysdepend/ra_fsp/lib/libtm/ek_ra8p1/tm_com.c) drives SCI8 by direct register writes,
 * and this demo's console is the FSP r_sci_b_uart driver on the SAME SCI8 channel
 * (ra_gen/hal_data.c, .channel = 8). Two drivers on one peripheral would fight.
 * USE_SYSTEM_MESSAGE and USE_EXCEPTION_DBG_MSG below are all guarded by
 * "&& USE_TMONITOR", so they become inert and are left unchanged. */
